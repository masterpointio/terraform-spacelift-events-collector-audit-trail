...
| cloudwatch\_logs\_retention\_days | Keep the Cloudwatch logs of the infrastructure (Lambdas & Kinesis Firehose) for this number of days. | `number` | `14` | no |
...
| lambda\_logs\_verbose | Include debug information in the Lambdas processing logs. | `bool` | `false` | no |
...
| Name | Description |
|------|-------------|
| audit\_trail\_storage\_bucket\_name | The name for the S3 bucket that stores the events |
